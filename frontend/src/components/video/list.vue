    ...
    <v-card color="background" elevation="0" class="pb-2">
        <v-row dense class="mb-2">
            ...
            <v-col v-for="(video, index) in videoList" :key="index" xs="12" sm="6" md="4" lg="3" xl="2">
                <VideoCard @click="playVideo(video)" :video-info="video" :hide-user-name="hideCardUserName" />
            </v-col>
        ...
        <v-dialog :model-value="videoDialog" fullscreen transition="dialog-bottom-transition">
            <v-card v-if="currentVideo">
                <Video
                  ...
                  :stats-only-actions="statsOnlyActions"
                />
            </v-card>
        </v-dialog>
    </v-card>
...
const { videoList, noDataMsg, showHot, simpleMode, hideAuthor, statsOnlyActions, hideCardUserName } = defineProps({
    videoList: {
        type: Object,
        default: []
    },
...
onMounted(() => {
    if (showHot) {
        apiVideoHotRank().then(({ data }) => {
            ...
            rankList.value = list.slice(0, 10)
        })
    }
})
